...
# Modules: GFCF effects, expert projections of selected technological and socioeconomic parameters for wind onshore, solar PV and bioenergy
# Scenarios, expert projections and output indicators and categories: defined by the user
##########################################################################


##########################################################################
# 0. Define conditions, set path, install packages and libraries, load data and functions / Definování podmínek, nastavení cesty, instalace balíèkù a knihoven, naètení vstupních dat a funkcí
##########################################################################

##########################################################################
# 0.0. Define conditions and set path / Definování podmínek a nastavení cesty

## Basic user options - 1 = yes, 0 = no: / Základní možnosti uživatele - 1 = ano, 0 = ne:

## Install libraries (packages) / Instalace knihoven (balíèkù)
install_libraries <- 0 # Does the user want to install the appropriate libraries? / Chce uživatel instalovat pøíslušné knihovny?

## Specifications for input conditions / Specifikace vstupních podmínek
p1 <- 0 # Does the user specify expert projections? / Zadává uživatel expertní projekce?
p2 <- 0 # Does the user model the whole energy sector? / Modeluje uživatel celý energetický sektor?
p3 <- 0 # Does the user enter inputs in percentages (0 means input in native values)? / Zadává uživatel vstupy v procentech (0 znamená zadávání v nativních hodnotách)?

## Specifications for output conditions (indicators) / Specifikace výstupních podmínek (indikátory)
i1 <- 0 # Does the user want to assess employment impacts? / Chce uživatel vyhodnotit dopady na zamìstnanost?
i2 <- 0 # Does the user want to assess the impacts on gross value added? / Chce uživatel vyhodnotit dopady na hrubou pøidanou hodnotu?
i3 <- 0 # Does the user want to assess the impact on gross domestic product? / Chce uživatel vyhodnotit dopady na hrubý domácí produkt?
i4 <- 0 # Does the user want to assess the impact on greenhouse gas emissions? / Chce uživatel vyhodnotit dopady na emise skleníkových plynù?

## Specifications for output categories / Specifikace výstupních kategorií
k1 <- 0 # Does the user want the total results? / Chce uživatel celkové výsledky?
k2 <- 0 # Does the user want results by effects (operation and maintenance vs. capital costs)? / Chce uživatel výsledky po jednotlivých fázích (provoz a údržba vs. kapitálové náklady)?
k3 <- 0 # Does the user want results by skill level and gender? / Chce uživatel výsledky po úrovních kvalifikace a podle pohlaví?
k4 <- 0 # Does the user want results by sector? / Chce uživatel výsledky po jednotlivých sektorech?
k5 <- 0 # Does the user want results by energy source in electricity generation? (only if p2==0) / Chce uživatel výsledky po jednotlivých zdrojích energie ve výrobì elektøiny? (jen pokud p2==0)
k6 <- 0 # Does the user want results by fuel type? (only if p2==1) / Chce uživatel výsledky po jednotlivých typech paliv? (jen pokud p2==1)


## Set datapath (for loading inputs) and respath (for saving results) according to the user's depository / Nastavení datapath (pro naèítání vstupù) a respath (pro ukládání výsledkù) podle úložištì uživatele
## Caution, you need to write the opposite slashes than the path copy from the explorer / Pozor, je potøeba psát opaènì lomítka, než je kopie cesty z prùzkumníka

# datapath = "C:/Users/Cerny/Documents/Work/Projects/2018-2021 TAÈR Théta IMPACTECH/WP6 Model a web/Model/Data/" # example
# respath <- "C:/Users/Cerny/Documents/Work/Projects/2018-2021 TAÈR Théta IMPACTECH/WP6 Model a web/Model/Results/" # example


##########################################################################
# 0.1. Install packages and libraries / Instalace balíèkù a knihoven
...
## Translate shares from projection_opex to opex_CZ
opex_CZ$Source_Category <- paste0(opex_CZ$Source,"_",opex_CZ$Category)
opex_CZ[,c(4:11)] <- projection_opex[match(opex_CZ$Source_Category,projection_opex$Source_Category),startsWith(colnames(projection_opex),"20")]
...
for (year in years) {
  ...
  ## (without the bioenergy depreciation row from the bioenergy column in opex, otherwise it pops up in the colsums of temp)
  ...
  ## Merge values from temp (derived from opex_CZ) with concordance2015_opex
  ## ERROR: In cbind(x[ids$x, , drop = FALSE], y[ids$y, y.cols, drop = FALSE]) : number of rows of result is not a multiple of vector length (arg 1)
...
## Domestic: Country.Code = CZ

## Note that I can only calculate "pure" domestic = where Country.Destination matches Country.Code (i.e. country of origin).
...
emp_sectors$Industry.Name <- c(rep.int(industries$cz, times = 8))
...
plot_sectors_av <- ggplot(emp_sectors_av, aes(Industry.Name, Average, fill = as.factor(desc(Industry.Name)))) +
  ...
  geom_text(aes(y = Average + 0.5 * sign(Average), label = round(Average, digits = 1))) +
  theme(axis.title.x = element_text(face = "bold"),
        axis.title = element_text(size = 16, face = "bold"),
...
emp_sources <- emp_sources %>% 
  filter(Country.Origin %in% "CZ") %>%
  filter(Extension %in% "emp") %>%
  group_by(Country.Code, Country.Destination, Year, Electricity) %>%
  summarize(Value = sum(Value))
...
## Rename sectors in Czech
gva_sectors$Industry.Name <- c(rep.int(industries$cz, times = 8))
...
plot_sectors_av <- ggplot(gva_sectors_av, aes(Industry.Name, Average, fill = as.factor(desc(Industry.Name)))) +
  ...
  ggtitle("Hrubá pøidaná hodnota - elektroenergetika a dodavatelské øetìzce",
          "10 odvìtví s nejvìtším nárùstem a poklesem (prùmìrná hodnota mezi roky 2015-2050)") +
  scale_fill_manual(values = c(rep(c("#2DAF6A"), times = 10),rep(c("#68002B"), times = 10))) + ## colours
  geom_text(aes(y = Average + 2500 * sign(Average), label = round(Average, digits = 1))) +
  theme(axis.title.x = element_text(face = "bold"),
        axis.title = element_text(size = 16, face = "bold"),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 11),
        axis.text.y = element_text(size = 14),
...
## Rename sectors in Czech
gdp_sectors$Industry.Name <- c(rep.int(industries$cz, times = 8))
...
print(plot_sources)
dev.off()
} # end of condition for 10.13
} # end of condition p2 for 10
...
##########################################################################
# 11.4. Organise EMP results by each fuel and plot
if (i1==1 && k6==1) {
## Aggregate results by each fuel (and nothing else)
emp_fuel <- results
emp_fuel <- emp_fuel %>% 
  filter(Country.Origin %in% "CZ") %>%
  filter(Extension %in% "emp") %>%
...
##########################################################################
# 11.7. Organise GVA results by each fuel and plot
if (i2==1 && k6==1) {
## Aggregate results by each fuel (and nothing else)
gva_fuel <- results
gva_fuel <- gva_fuel %>% 
...
##########################################################################
# 11.10. Organise GDP results by each fuel and plot
if (i3==1 && k6==1) {
## Aggregate results by each fuel (and nothing else)
gdp_fuel <- results
gdp_fuel <- gdp_fuel %>% 
...
##########################################################################
# 11.12. Organise GHG results by each fuel and plot
if (i4==1 && k6==1) {
## Aggregate results by each fuel (and nothing else)
ghg_fuel <- results
ghg_fuel <- ghg_fuel %>%
