...
##########################################################################
# 0. Define conditions, set path, install packages and libraries, load data and functions
##########################################################################

##########################################################################
# 0.0. Define conditions and set path
...
p2 <- 1 # Does the user model the whole energy sector? / Modeluje uživatel celý energetický sektor?
...
i1 <- 1 # Does the user want to assess employment impacts? / Chce uživatel vyhodnotit dopady na zamìstnanost?
...
k2 <- 1 # Does the user want results by effects (operation and maintenance vs. capital costs)? / Chce uživatel výsledky po jednotlivých fázích (provoz a údržba vs. kapitálové náklady)?
k3 <- 1 # Does the user want results by skill level and gender? / Chce uživatel výsledky po úrovních kvalifikace a podle pohlaví?
k4 <- 1 # Does the user want results by sector? / Chce uživatel výsledky po jednotlivých sektorech?
...
respath <- "C:/Users/Cerny/Documents/Work/Projects/2018-2021 TAÈR Théta IMPACTECH/WP6 Model a web/Model/Results/Experiments/" # example
...
## Read scenario in absolute values for the production of electricity by each source
...
## Read scenario in absolute values for the installed capacity of each electricity source
...
emp_sources <- emp_sources %>% 
  filter(Country.Origin %in% "CZ") %>%
  filter(Extension %in% "emp") %>%
  group_by(Country.Destination, Year, Electricity) %>%
  summarize(Value = sum(Value))
